<div align="center">

# Eddy Zhang

**AI full-stack engineer building production AI systems**<br/>
<sub>Sydney, Australia · RAG systems · LLM agents · Spring Cloud · Next.js</sub>

<sup>
  <a href="https://eddyzhang.me/">Portfolio</a>
  &nbsp;·&nbsp;
  <a href="https://linkedin.com/in/eddy-shousen-zhang">LinkedIn</a>
  &nbsp;·&nbsp;
  <a href="mailto:eddy.zhang24@gmail.com">Email</a>
  &nbsp;·&nbsp;
  <a href="https://www.joblit.tech/">Joblit</a>
</sup>

</div>

---

I build AI-native products where prompts, retrieval, model routing, evaluation, backend reliability, and product UX are treated as one production system. My work sits at the intersection of applied LLM engineering, full-stack product delivery, and cloud-native system design.

<table align="center">
  <tr>
    <td width="33%" valign="top">
      <strong>Current focus</strong><br/>
      RAG-powered agents, portable AI skill packs, eval-driven prompting, and multi-provider LLM orchestration.
    </td>
    <td width="33%" valign="top">
      <strong>Engineering base</strong><br/>
      Java / Spring Cloud services, TypeScript frontends, Postgres-backed product systems, and CI/CD delivery.
    </td>
    <td width="33%" valign="top">
      <strong>Operating style</strong><br/>
      Ship useful software, keep model behavior observable, and turn fragile prompt glue into reusable systems.
    </td>
  </tr>
</table>

## Selected Work

<table>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://www.joblit.tech/">Joblit</a></h3>
      <p><strong>AI job-search platform</strong> built around Claude Code, structured prompt workflows, and multi-provider LLM routing.</p>
      <ul>
        <li>Portable AI skill pack with versioned instructions and quality gates.</li>
        <li>Claude, GPT, and Gemini routing for resilient generation flows.</li>
        <li>Chrome Extension (Manifest V3) for ATS form assistance.</li>
        <li>Next.js, TypeScript, Prisma, Postgres, and Vercel delivery.</li>
      </ul>
      <sub><code>Next.js</code> · <code>TypeScript</code> · <code>Prisma</code> · <code>Claude</code> · <code>MV3</code></sub>
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/ShousenZHANG/project-contest-platform">Competition Platform</a></h3>
      <p><strong>Spring Cloud microservices platform</strong> with JWT SSO, RabbitMQ event flow, RBAC, and audit-oriented backend design.</p>
      <ul>
        <li>Runner-up, Coding Fest 2025 at the University of Sydney.</li>
        <li>7+ services across auth, contest, submission, and operations flows.</li>
        <li>RabbitMQ topics for asynchronous event processing.</li>
        <li>Docker-composed local stack with AWS deployment targets.</li>
      </ul>
      <sub><code>Java</code> · <code>Spring Cloud</code> · <code>MySQL</code> · <code>RabbitMQ</code> · <code>Docker</code> · <code>AWS</code></sub>
    </td>
  </tr>
</table>

## Activity Skyline

<p align="center">
  <img src="https://raw.githubusercontent.com/ShousenZHANG/ShousenZHANG/output/github-activity-3d.svg" alt="3D GitHub contribution skyline" width="100%" />
</p>

## Contribution Snake

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/ShousenZHANG/ShousenZHANG/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/ShousenZHANG/ShousenZHANG/output/github-snake.svg" />
    <img alt="GitHub contribution snake" src="https://raw.githubusercontent.com/ShousenZHANG/ShousenZHANG/output/github-snake-dark.svg" width="100%" />
  </picture>
</p>

---

<div align="center">
  <sub>Sydney · Always shipping · Open to AI engineering opportunities</sub>
</div>
